            ...
            <?php if ($data['stock'] > 0): ?>
              <div class="infoPriceProduct">
                <div class="case_quantity_wanted">
                  ...
                  <button onclick="addToCart('materiel', '<?=$id?>', '<?= $data['name'] ?>', document.getElementById('<?= $data['name'] ?>-add-qty').value, document.getElementById('<?= $data['name'] ?>-add-qty').name)" class="favorite styled">Ajouter au panier</button>
                </div>
              </div>
            <?php endif; ?>
          <?php endforeach; ?>
