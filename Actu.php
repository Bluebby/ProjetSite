...
<body>
    <header id="showcase">
        ...
        <div id="responsive">
            ...
            <div class="product">
                <h1 class="titleProduct" style="color:rgb(255, 92, 82)">PRODUIT A VENIR</h1>
                <div class="underProduct">

                    <!-- Affichage des produits à partir de la base de données. -->
                    <?php foreach ($_SESSION['products_data']['Actu'] as $id => $data) : ?>
                        <?php if ($data['stock'] > 0) : ?>
                            <div class="infoPriceProduct">
                                <div class="case_quantity_wanted">
                                    ...
                                    <button onclick="addToCart('plantes', '<?= $id ?>', '<?= $data['name'] ?>', document.getElementById('<?= $data['name'] ?>-add-qty').value, document.getElementById('<?= $data['name'] ?>-add-qty').name)" class="favorite styled">Précommander</button>
                                </div>
                            </div>
                        <?php endif; ?>
                    <?php endforeach; ?>
